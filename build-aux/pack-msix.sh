...
#
# It takes the tree bundle-windows.sh leaves (bin/office42.exe and the lib
...
# it on this machine instead, set O42_MSIX_CERT_PFX to a self-signed
...
# name Office42, so a plain run makes the package the Store expects.  These
# override it, for a different product or a different account:
#
#   O42_MSIX_IDENTITY_NAME      Package/Identity/Name
#   O42_MSIX_PUBLISHER          Package/Identity/Publisher (CN=<GUID>)
#   O42_MSIX_PUBLISHER_DISPLAY  PublisherDisplayName
#
set -euo pipefail
...
msix_version=${O42_MSIX_VERSION:-$(awk -F. '{printf "%d.%d.%d.0", $1, $2, $3}' <<<"${version%%-*}")}

# The identity Partner Center assigned when the name Office42 was reserved.
...
msix=$root/$outdir/office42-$version-win64.msix
...
if [ -n "${O42_MSIX_CERT_PFX:-}" ]; then
  signtool=$(kit_tool signtool.exe) || {
    echo "pack-msix: O42_MSIX_CERT_PFX is set but signtool.exe was not found" >&2
    exit 1
  }
  MSYS2_ARG_CONV_EXCL='*' "$signtool" sign /fd SHA256 /a \
    /f "$(winpath "$O42_MSIX_CERT_PFX")" \
    ${O42_MSIX_CERT_PASS:+/p "$O42_MSIX_CERT_PASS"} "$(winpath "$msix")"
fi
