...
const O42FunctionHelp O42_HELP_RANDOM[] = {
  { "RANDBERNOULLI", "RANDBERNOULLI(p)", N_("One or nothing, with the given chance of one.") },
  { "RANDBETA", "RANDBETA(a, b)", N_("A random number from the beta distribution.") },
  { "RANDBINOM", "RANDBINOM(p, trials)", N_("How many successes in so many trials.") },
  { "RANDCAUCHY", "RANDCAUCHY(a)", N_("A random number from Cauchy's distribution.") },
  { "RANDCHISQ", "RANDCHISQ(df)", N_("A random number from the chi-squared distribution.") },
  { "RANDDISCRETE", "RANDDISCRETE(values, probabilities)", N_("One of the values, drawn with the given weights.") },
  { "RANDEXP", "RANDEXP(b)", N_("A random number from the exponential distribution.") },
  { "RANDEXPPOW", "RANDEXPPOW(a, b)", N_("A random number from the exponential power distribution.") },
  { "RANDFDIST", "RANDFDIST(df1, df2)", N_("A random number from Fisher's F distribution.") },
  { "RANDGAMMA", "RANDGAMMA(shape, scale)", N_("A random number from the gamma distribution.") },
  { "RANDGEOM", "RANDGEOM(p)", N_("How many trials until the first success.") },
  { "RANDGUMBEL1", "RANDGUMBEL1(a, b)", N_("A random number from Gumbel's first distribution.") },
  { "RANDGUMBEL2", "RANDGUMBEL2(a, b)", N_("A random number from Gumbel's second distribution.") },
  { "RANDHYPERG", "RANDHYPERG(n1, n2, t)", N_("How many of the first kind in a draw from an urn.") },
  { "RANDLAPLACE", "RANDLAPLACE(a)", N_("A random number from Laplace's distribution.") },
  { "RANDLEVY", "RANDLEVY(c, alpha, beta)", N_("A random number from Levy's alpha-stable distribution.") },
  { "RANDLOG", "RANDLOG(p)", N_("A random number from the logarithmic distribution.") },
  { "RANDLOGARITHMIC", "RANDLOGARITHMIC(p)", N_("A random number from the logarithmic distribution.") },
  { "RANDLOGISTIC", "RANDLOGISTIC(a)", N_("A random number from the logistic distribution.") },
  { "RANDLOGNORM", "RANDLOGNORM(zeta, sigma)", N_("A random number from the log-normal distribution.") },
  { "RANDNEGBINOM", "RANDNEGBINOM(p, failures)", N_("How many successes before so many failures.") },
  { "RANDNORM", "RANDNORM(mean, stdev)", N_("A random number from the normal distribution.") },
  { "RANDNORMTAIL", "RANDNORMTAIL(a, sigma)", N_("A random number from the tail of a normal distribution above a.") },
  { "RANDPARETO", "RANDPARETO(a, b)", N_("A random number from Pareto's distribution.") },
  { "RANDPOISSON", "RANDPOISSON(lambda)", N_("A random number from Poisson's distribution.") },
  { "RANDRAYLEIGH", "RANDRAYLEIGH(sigma)", N_("A random number from Rayleigh's distribution.") },
  { "RANDRAYLEIGHTAIL", "RANDRAYLEIGHTAIL(a, sigma)", N_("Rayleigh's distribution from a upwards.") },
  { "RANDSNORM", "RANDSNORM()", N_("A random number from the standard normal distribution.") },
  { "RANDSTNORM", "RANDSTNORM()", N_("A random number from the standard normal distribution.") },
  { "RANDTDIST", "RANDTDIST(df)", N_("A random number from Student's t distribution.") },
  { "RANDUNIFORM", "RANDUNIFORM(lower, upper)", N_("A random number between the two, evenly.") },
  { "RANDWEIBULL", "RANDWEIBULL(a, b)", N_("A random number from Weibull's distribution.") },
  { NULL, NULL, NULL }
};
