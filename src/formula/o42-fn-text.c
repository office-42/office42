...
const O42FunctionHelp O42_HELP_TEXT[] = {
  { "LENB", "LENB(text)", "How many characters a text has; the same as LEN here." },
  { "LEFTB", "LEFTB(text, count)", "The first characters of a text; the same as LEFT here." },
  { "RIGHTB", "RIGHTB(text, count)", "The last characters of a text; the same as RIGHT here." },
  { "MIDB", "MIDB(text, start, count)", "Characters from the middle of a text; the same as MID here." },
  { "FINDB", "FINDB(find_text, within_text, start)", "Where one text starts inside another; the same as FIND here." },
  { "SEARCHB", "SEARCHB(find_text, within_text, start)", "Where one text starts inside another; the same as SEARCH here." },
  { "REPLACEB", "REPLACEB(old_text, start, count, new_text)", "Replaces part of a text by position; the same as REPLACE here." },
  { "JIS", "JIS(text)", "Halfwidth letters, digits and marks made fullwidth." },
  { "DBCS", "DBCS(text)", "Halfwidth letters, digits and marks made fullwidth; Excel's newer name for JIS." },
  { "ASC", "ASC(text)", "Fullwidth letters, digits and marks made halfwidth." },
  { "REGEXTEST", "REGEXTEST(text, pattern, [case])", "Whether a regular expression matches somewhere in the text." },
  { "REGEXREPLACE", "REGEXREPLACE(text, pattern, replacement, [occurrence], [case])", "The text with what a regular expression matches replaced; \\1 for a group." },
  { "PERCENTOF", "PERCENTOF(subset, all)", "The sum of a subset as a share of the sum of the whole." },
  { "ENCODEURL", "ENCODEURL(text)", "The text as it goes into a URL, %XX for what is not a letter or digit." },
