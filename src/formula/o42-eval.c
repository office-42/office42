...
/* A union, (A1:A2,C1:C2): several areas that travel as one operand.
 * The operand is a range on a sheet named \002 and the union's index,
 * showing its first area, and the areas are kept here. */
typedef struct {
  GArray *areas;     /* O42Operand, each a range */
} UnionAreas;
...
static void
union_areas_free (UnionAreas *u)
{
  g_array_free (u->areas, TRUE);
  g_free (u);
}

/* The areas of a union operand, or NULL for an ordinary one. */
static GArray *
union_areas (const O42Operand *op)
{
  ArrayFrame *frame = array_frames && array_frames->len > 0
                      ? g_ptr_array_index (array_frames, array_frames->len - 1) : NULL;
  guint idx;

  if (!op->is_range || op->sheet == NULL || op->sheet[0] != '\002' || frame == NULL)
    return NULL;
  idx = (guint) atoi (op->sheet + 1);
  return idx < frame->unions->len ? ((UnionAreas *) g_ptr_array_index (frame->unions, idx))->areas : NULL;
}
...
array_get_cell (O42EvalContext *ctx, const char *sheet, int row, int col, O42Value *out)
{
  ...
  if (sheet != NULL && sheet[0] == '\002' && frame != NULL)
    {
      /* A union's cell is read from its first area's sheet. */
      guint idx = (guint) atoi (sheet + 1);
      if (idx < frame->unions->len)
        {
          UnionAreas *u = g_ptr_array_index (frame->unions, idx);
          if (u->areas->len > 0)
            {
              const O42Operand *first = &g_array_index (u->areas, O42Operand, 0);
              array_get_cell (ctx, first->sheet, row, col, out);
              return;
            }
        }
...
reference_operator (O42EvalContext *ctx, O42Op op, const O42Operand *oa, const O42Operand *ob)
{
  ...
  if (!oa->is_range || !ob->is_range)
    {
      ...
      if (!oa->is_range && oa->value.type == O42_VALUE_ERROR)
        ...
        { o42_value_clear (&out.value); out.value = o42_value_copy (&ob->value); }
      return out;
    }
  ...
  if (op == O42_OP_UNION)
    {
      ...
      u = g_new0 (UnionAreas, 1);
      u->areas = g_array_new (FALSE, TRUE, sizeof (O42Operand));
      for (int k = 0; k < 2; k++)
        {
          GArray *inner = union_areas (sides[k]);

          if (inner != NULL)
            for (guint i = 0; i < inner->len; i++)
              g_array_append_val (u->areas, g_array_index (inner, O42Operand, i));
          else
            {
              O42Operand area = *sides[k];
              area.value = o42_value_empty ();
              g_array_append_val (u->areas, area);
            }
        }
      g_ptr_array_add (frame->unions, u);
      name = g_strdup_printf ("\002%u", frame->unions->len - 1);
      out = g_array_index (u->areas, O42Operand, 0);
  ...
  /* The intersection: the same sheet, and an overlap. */
    ...
    }
  {
    ...
    r.row0 = MAX (oa->range.row0, ob->range.row0);
    r.col0 = MAX (oa->range.col0, ob->range.col0);
    r.row1 = MIN (oa->range.row1, ob->range.row1);
    r.col1 = MIN (oa->range.col1, ob->range.col1);
    if (r.row0 > r.row1 || r.col0 > r.col1)
      {
        out.value = o42_value_error (O42_ERR_NULL);
        return out;
      }
    out.is_range = TRUE;
    out.sheet = oa->sheet;
...
/* @A1:A3: the one cell of a column that is in the formula's row, or of
 * a row that is in its column; a single cell or a value is itself, and
 * a block is #VALUE!. */
static O42Operand
implicit_intersection (O42EvalContext *ctx, const O42Operand *oa)
{
  O42Operand out;
  int rows, cols;

  memset (&out, 0, sizeof out);
  if (!oa->is_range || union_areas (oa) != NULL)
    {
      if (oa->is_range)
        out.value = o42_value_error (O42_ERR_VALUE);
      else
        out.value = o42_value_copy (&oa->value);
      return out;
    }
  operand_dims (oa, &rows, &cols);
  out = *oa;
  out.value = o42_value_empty ();
  if (rows == 1 && cols == 1)
    return out;
  if (cols == 1 && ctx->row >= oa->range.row0 && ctx->row <= oa->range.row1)
    {
      out.range.row0 = out.range.row1 = ctx->row;
      return out;
    }
  if (rows == 1 && ctx->col >= oa->range.col0 && ctx->col <= oa->range.col1)
    {
      out.range.col0 = out.range.col1 = ctx->col;
      return out;
    }
  memset (&out, 0, sizeof out);
  out.value = o42_value_error (O42_ERR_VALUE);
...
o42_operand_value (O42EvalContext *ctx, const O42Operand *op)
{
  ...
  if (!op->is_range)
    return o42_value_copy (&op->value);
  if (op->sheet != NULL && op->sheet[0] == '\002')
...
fn_lookup_vector (O42EvalContext *ctx, O42Operand *args, int n)
{
  ...
  /* A vector is searched along its length; the array form searches
   * the first column when it is taller than wide, else the first row. */
  vertical = (n < 3) ? (from->row1 - from->row0 > from->col1 - from->col0 || from->col0 == from->col1)
                     : (from->col0 == from->col1);
  ...
  /* The array form, one range of several rows and columns: the result
   * comes from the last column, or the last row. */
  if (n < 3 && from->col0 != from->col1 && from->row0 != from->row1)
    {
      if (vertical)
        ctx->get_cell (ctx, args[1].sheet, from->row0 + found, from->col1, &result);
      else
        ctx->get_cell (ctx, args[1].sheet, from->row1, from->col0 + found, &result);
      return result;
    }
...
fn_address (O42EvalContext *ctx, O42Operand *args, int n)
{
  ...
  if (n >= 4)
    {
      /* a1 FALSE asks for R1C1: R2C3, R2C[3], R[2]C3, R[2]C[3], the
       * brackets marking what is relative. */
      gboolean a1 = TRUE;
      O42Value style = operand_value (ctx, &args[3]);
      O42ErrorCode e = O42_ERR_VALUE;

      if (style.type == O42_VALUE_ERROR)
        return style;
      if (style.type != O42_VALUE_EMPTY && !o42_value_to_bool (&style, &a1, &e))
        { o42_value_clear (&style); return o42_value_error (e); }
      o42_value_clear (&style);
      if (!a1)
        ref = g_strdup_printf (abs == 1 ? "R%dC%d" : abs == 2 ? "R%dC[%d]" : abs == 3 ? "R[%d]C%d" : "R[%d]C[%d]",
                               (int) row, (int) col);
      else
...
/* One R1C1 reference -- R3C1, R[1]C[-1], R, C2 -- as a row and column,
 * relative parts taken from the formula's own cell; NULL past it. */
static const char *
r1c1_one (const char *p, int row, int col, int *out_row, int *out_col)
{
  int r = row, c = col;

  if (g_ascii_toupper (*p) != 'R')
    return NULL;
  p++;
  if (*p == '[')
    {
      char *end;
      r = row + (int) strtol (p + 1, &end, 10);
      if (*end != ']') return NULL;
      p = end + 1;
    }
  ...
  if (g_ascii_toupper (*p) != 'C')
    return NULL;
  p++;
  if (*p == '[')
    {
      char *end;
      c = col + (int) strtol (p + 1, &end, 10);
      if (*end != ']') return NULL;
      p = end + 1;
    }
  else if (g_ascii_isdigit (*p))
...
static char *
r1c1_to_a1 (const char *text, int row, int col)
{
  const char *bang = strrchr (text, '!');
  const char *p = bang != NULL ? bang + 1 : text;
  int r0, c0, r1, c1;
  char *a, *b, *result;

  p = r1c1_one (p, row, col, &r0, &c0);
  if (p == NULL)
    return NULL;
  a = o42_ref_name (r0, c0);
  if (*p == ':')
    {
      p = r1c1_one (p + 1, row, col, &r1, &c1);
      if (p == NULL || *p != '\0')
        { g_free (a); return NULL; }
      b = o42_ref_name (r1, c1);
      result = g_strdup_printf ("%.*s%s:%s", bang != NULL ? (int) (bang - text + 1) : 0, text, a, b);
      g_free (b);
    }
  else if (*p != '\0')
    { g_free (a); return NULL; }
  else
    result = g_strdup_printf ("%.*s%s", bang != NULL ? (int) (bang - text + 1) : 0, text, a);
  g_free (a);
  return result;
}
...
#undef SPLIT_AT
  ...
  if (strcmp (node->as.call.name, "CHOOSE") == 0 && n_args >= 2)
    {
      /* CHOOSE hands back the argument itself, range and all, so that
       * SUM(CHOOSE(2,A1:A2,A3:A4)) adds the second range; an array of
       * indices is left to the lifted call. */
      O42Operand which = eval_operand (ctx, g_ptr_array_index (node->as.call.args, 0));
      O42Value v;
      double index;
      O42ErrorCode e = O42_ERR_VALUE;
      ...
      operand_clear (&which);
      if (v.type == O42_VALUE_ERROR)
        { *out = (O42Operand) { 0 }; out->value = v; return TRUE; }
  ...
  if (strcmp (node->as.call.name, "AREAS") == 0 && n_args == 1)
    {
      O42Operand ref = eval_operand (ctx, g_ptr_array_index (node->as.call.args, 0));
      GArray *areas = union_areas (&ref);

      out->value = !ref.is_range ? o42_value_error (O42_ERR_VALUE)
                 : o42_value_number (areas != NULL ? (double) areas->len : 1);
      operand_clear (&ref);
      return TRUE;
    }

  if (strcmp (node->as.call.name, "INDEX") == 0 && n_args >= 1 && n_args <= 4)
    {
      /* INDEX gives back a reference: a cell, or with a row or column
       * of 0 the whole column or row, which SUM adds and a cell spills;
       * the fourth argument picks an area of a union. */
      O42Operand src = eval_operand (ctx, g_ptr_array_index (node->as.call.args, 0));
      GArray *areas = union_areas (&src);
      O42Operand nums[3];
      double row = 1, col = 1, area = 1;
      double *targets[3] = { &row, &col, &area };
      int rows, cols;
      ...
      for (int k = 1; k < n_args; k++)
        {
          ...
          operand_clear (&nums[k - 1]);
          if (v.type == O42_VALUE_EMPTY)
            { o42_value_clear (&v); *targets[k - 1] = k == 3 ? 1 : 0; continue; }
          if (v.type == O42_VALUE_ERROR)
            { operand_clear (&src); *out = (O42Operand) { 0 }; out->value = v; return TRUE; }
          if (!o42_value_to_number (&v, targets[k - 1], &e))
            { o42_value_clear (&v); operand_clear (&src); out->value = o42_value_error (e); return TRUE; }
          o42_value_clear (&v);
        }
      if (areas != NULL)
        {
          if (area < 1 || area > areas->len)
            { operand_clear (&src); out->value = o42_value_error (O42_ERR_REF); return TRUE; }
          src = g_array_index (areas, O42Operand, (int) area - 1);
          src.value = o42_value_empty ();
        }
      operand_dims (&src, &rows, &cols);
      /* INDEX(A1:F1, 3) on a single row means the third column. */
      if (n_args == 2 && rows == 1 && cols > 1)
        { col = row; row = 1; }
      row = floor (row);
      col = floor (col);
      ...
      *out = src;
      out->value = o42_value_empty ();
      if (row > 0)
        out->range.row0 = out->range.row1 = src.range.row0 + (int) row - 1;
      if (col > 0)
        out->range.col0 = out->range.col1 = src.range.col0 + (int) col - 1;
      return TRUE;
    }

  if (strcmp (node->as.call.name, "COUNTBLANK") == 0 && n_args == 1)
    {
      /* A whole column is walked only over the sheet's stored cells;
       * the rows beyond them are blank too, and COUNTBLANK(A:A) on an
       * empty column is the million rows it has. */
      const O42Node *arg = g_ptr_array_index (node->as.call.args, 0);

      if (arg->type == O42_NODE_RANGE && (arg->abs & (O42_WHOLE_COLS | O42_WHOLE_ROWS)))
        {
          O42Operand ref = eval_operand (ctx, arg);
          double total = (double) (arg->as.range.row1 - arg->as.range.row0 + 1)
                         * (double) (arg->as.range.col1 - arg->as.range.col0 + 1);
          double filled = 0;

          if (ref.is_range)
            for (int r = ref.range.row0; r <= ref.range.row1; r++)
              for (int c = ref.range.col0; c <= ref.range.col1; c++)
                {
                  O42Value v;
                  ctx->get_cell (ctx, ref.sheet, r, c, &v);
                  if (!(v.type == O42_VALUE_EMPTY || (v.type == O42_VALUE_TEXT && *v.as.text == '\0')))
                    filled++;
                  o42_value_clear (&v);
                }
          operand_clear (&ref);
          out->value = o42_value_number (total - filled);
          return TRUE;
        }
      return FALSE;
    }
  ...
  if (strcmp (node->as.call.name, "INDIRECT") == 0)
    {
      ...
      if (n_args == 2)
        {
          O42Value style = eval_node (ctx, g_ptr_array_index (node->as.call.args, 1));
          O42ErrorCode e = O42_ERR_VALUE;
          if (style.type != O42_VALUE_EMPTY && !o42_value_to_bool (&style, &a1, &e))
      ...
      if (!a1)
        {
          /* R3C1, R[1]C[-1], R3C1:R5C2: turned into A1 form, relative
           * offsets from the formula's own cell. */
          char *converted = r1c1_to_a1 (v.as.text, ctx->row, ctx->col);
          o42_value_clear (&v);
          if (converted == NULL)
            { out->value = o42_value_error (O42_ERR_REF); return TRUE; }
          v = o42_value_take (converted);
        }
...
static const struct {
  ...
  { "FOURIER", "FOURIER(sequence, inverse, separate)", N_("Gnumeric's: the Fourier transform of a sequence, or its inverse.") },
  { "FREQUENCY", "FREQUENCY(data, bins)", N_("How many values fall in each bin.") },
  { "HPFILTER", "HPFILTER(sequence, lambda)", N_("Gnumeric's: Hodrick and Prescott's trend and what is left over.") },
  { "INTERPOLATION", "INTERPOLATION(abscissae, ordinates, targets, method)", N_("Gnumeric's: the ordinates read off at the targets, six ways.") },
  { "LEVERAGE", "LEVERAGE(array)", N_("Gnumeric's: how much each row of a design matrix pulls on the fit.") },
  { "LINEST", "LINEST(known_y, known_x, const, stats)", N_("The least-squares line, with its statistics.") },
  { "LOGFIT", "LOGFIT(known_ys, known_xs)", N_("Gnumeric's fit of y = a + b ln(sign (x - c)): sign, a, b, c and the residuals.") },
  { "LOGREG", "LOGREG(known_ys, known_xs, const, stats)", N_("Gnumeric's: the least-squares fit of y = m ln(x) + b, with its statistics.") },
  { "MINVERSE", "MINVERSE(array)", N_("The inverse of a square matrix.") },
  { "MMULT", "MMULT(array1, array2)", N_("The product of two matrices.") },
  { "PERIODOGRAM", "PERIODOGRAM(ordinates, filter, abscissae, method, number)", N_("Gnumeric's: how much of each frequency is in a series.") },
  { "TRANSPOSE", "TRANSPOSE(array)", N_("A rectangle turned on its side.") },
  { "CHOOSECOLS", "CHOOSECOLS(array, col1, col2, ...)", N_("The columns named, in the order named.") },
  { "CHOOSEROWS", "CHOOSEROWS(array, row1, row2, ...)", N_("The rows named, in the order named.") },
  { "TAKE", "TAKE(array, rows, cols)", N_("So many rows and columns from the near end, or the far one.") },
  { "VSTACK", "VSTACK(array1, array2, ...)", N_("The arrays one under another, short ones padded with #N/A.") },
  { "HSTACK", "HSTACK(array1, array2, ...)", N_("The arrays side by side, short ones padded with #N/A.") },
  { "EXPAND", "EXPAND(array, rows, [columns], [pad_with])", N_("The array grown to a size, the new cells #N/A or the padding.") },
  { "WRAPROWS", "WRAPROWS(vector, wrap_count, [pad_with])", N_("A vector folded into rows of so many.") },
  { "WRAPCOLS", "WRAPCOLS(vector, wrap_count, [pad_with])", N_("A vector folded into columns of so many.") },
  { "TOCOL", "TOCOL(array, [ignore], [scan_by_column])", N_("An array as one column, blanks or errors left out when asked.") },
  { "TOROW", "TOROW(array, [ignore], [scan_by_column])", N_("An array as one row, blanks or errors left out when asked.") },
  { "MUNIT", "MUNIT(dimension)", N_("The identity matrix of a size.") },
  { "TRIMRANGE", "TRIMRANGE(range, [trim_rows], [trim_cols])", N_("The range without its empty outer rows and columns.") },
  { "REGEXEXTRACT", "REGEXEXTRACT(text, pattern, [return_mode], [case])", N_("What a regular expression matches: the first match, all of them, or the groups.") },
  { "GETPIVOTDATA", "GETPIVOTDATA(data_field, pivot_table, ...)", N_("A value from a pivot table; #REF! here, whose pivots are plain cells.") },
  { "DROP", "DROP(array, rows, cols)", N_("The rectangle with so many rows and columns left off.") },
  { "TEXTSPLIT", "TEXTSPLIT(text, col_delimiter, [row_delimiter], [ignore_empty], [match_mode], [pad_with])", N_("A text cut into a rectangle at its delimiters.") },
  { "MODE.MULT", "MODE.MULT(number1, number2, ...)", N_("Every value that turns up as often as the commonest.") },
  { "CHOLESKY", "CHOLESKY(matrix)", N_("Gnumeric's: the lower triangle whose product with its transpose is the matrix.") },
  { "EIGEN", "EIGEN(matrix)", N_("Gnumeric's: the eigenvalues of a symmetric matrix, each with its vector under it.") },
  { "MPSEUDOINVERSE", "MPSEUDOINVERSE(matrix, tolerance)", N_("Gnumeric's: Moore and Penrose's inverse, which any matrix has.") }
};
...
static const struct {
  ...
  { "ABS", "ABS(number)", N_("The absolute value of a number.") },
  { "ACOS", "ACOS(number)", N_("The arccosine, in radians.") },
  { "ACOSH", "ACOSH(number)", N_("The inverse hyperbolic cosine.") },
  { "ACOT", "ACOT(number)", N_("The arccotangent, in radians.") },
  { "ACOTH", "ACOTH(number)", N_("The inverse hyperbolic cotangent.") },
  { "ADDRESS", "ADDRESS(row, column, abs_num, a1, sheet)", N_("A cell reference as text.") },
  { "ADTEST", "ADTEST(array)", N_("Anderson and Darling's test: the chance a sample this far from normal.") },
  { "AND", "AND(logical1, logical2, ...)", N_("TRUE if every argument is TRUE.") },
  { "ARABIC", "ARABIC(text)", N_("A Roman numeral as a number.") },
  { "AREAS", "AREAS(reference)", N_("The number of areas in a reference: one.") },
  { "ARRAYTOTEXT", "ARRAYTOTEXT(array, format)", N_("A rectangle as text, plainly or in braces.") },
  { "ASCENSIONTHURSDAY", "ASCENSIONTHURSDAY(year)", N_("Ascension Thursday, thirty-nine days after Easter.") },
  { "ASHWEDNESDAY", "ASHWEDNESDAY(year)", N_("Ash Wednesday, forty-six days before Easter.") },
  { "ASIN", "ASIN(number)", N_("The arcsine, in radians.") },
  { "ASINH", "ASINH(number)", N_("The inverse hyperbolic sine.") },
  { "ATAN", "ATAN(number)", N_("The arctangent, in radians.") },
  { "ATAN2", "ATAN2(x, y)", N_("The arctangent of y/x, in radians, in the right quadrant.") },
  { "ATANH", "ATANH(number)", N_("The inverse hyperbolic tangent.") },
  { "AVERAGE", "AVERAGE(number1, number2, ...)", N_("The arithmetic mean; text and blanks in a range are skipped.") },
  { "AVERAGEA", "AVERAGEA(value1, value2, ...)", N_("The average, counting text as 0 and TRUE as 1.") },
  { "AVERAGEIF", "AVERAGEIF(range, criteria, average_range)", N_("The mean of the cells that meet a condition.") },
  { "AVERAGEIFS", "AVERAGEIFS(average_range, range1, criteria1, ...)", N_("The mean of the cells meeting every condition.") },
  { "BASE", "BASE(number, radix, min_length)", N_("A number as text in another base.") },
  { "BETA", "BETA(a, b)", N_("The beta function.") },
  { "BETA.DIST", "BETA.DIST(x, alpha, beta, cumulative, A, B)", N_("The beta distribution, density or cumulative.") },
  { "BETALN", "BETALN(a, b)", N_("The logarithm of the beta function.") },
  { "BINOM.DIST.RANGE", "BINOM.DIST.RANGE(trials, probability_s, number_s, number_s2)", N_("The probability of a range of successes.") },
  { "BINOM.INV", "BINOM.INV(trials, probability_s, alpha)", N_("The smallest value whose cumulative binomial distribution reaches alpha.") },
  { "BYCOL", "BYCOL(array, lambda)", N_("Each column of an array through a function.") },
  { "BYROW", "BYROW(array, lambda)", N_("Each row of an array through a function.") },
  { "CAUCHY", "CAUCHY(x, a, cumulative)", N_("The Cauchy, Lorentz or Breit-Wigner distribution.") },
  { "CEIL", "CEIL(number)", N_("Rounded up to the next whole number.") },
  { "CEILING", "CEILING(number, significance)", N_("Rounds up, away from zero, to a multiple of significance.") },
  { "CEILING.MATH", "CEILING.MATH(number, significance, mode)", N_("Rounds up to a multiple of the significance.") },
  { "CEILING.PRECISE", "CEILING.PRECISE(number, significance)", N_("Rounded up towards plus infinity, whatever the sign.") },
  { "CHISQ.DIST", "CHISQ.DIST(x, deg_freedom, cumulative)", N_("The chi-squared distribution, density or cumulative.") },
  { "CHISQ.INV", "CHISQ.INV(probability, deg_freedom)", N_("The inverse of the left-tailed chi-squared distribution.") },
  { "CHISQ.TEST", "CHISQ.TEST(actual_range, expected_range)", N_("The chi-squared test for independence.") },
  { "CHISQDIST", "CHISQDIST(x, dof, cumulative)", N_("The chi-squared distribution, as Gnumeric names it.") },
  { "CHISQINV", "CHISQINV(p, dof)", N_("The value of chi-squared with the given probability below it.") },
  { "CHITEST", "CHITEST(actual_range, expected_range)", N_("The chi-squared test for independence.") },
  { "CHOOSE", "CHOOSE(index, value1, value2, ...)", N_("The value at a given position in the list.") },
  { "COLUMN", "COLUMN(reference)", N_("The column number of a reference, or of this cell.") },
  { "COMBIN", "COMBIN(n, k)", N_("How many ways k things can be chosen from n.") },
  { "COMBINA", "COMBINA(number, number_chosen)", N_("Combinations with repetition.") },
  { "CONFIDENCE.T", "CONFIDENCE.T(alpha, standard_dev, size)", N_("Half the width of a confidence interval, by Student's t.") },
  { "CONVERT", "CONVERT(number, from_unit, to_unit)", N_("A measurement in other units: CONVERT(1,\"mi\",\"km\").") },
  { "COS", "COS(number)", N_("The cosine of an angle in radians.") },
  { "COSH", "COSH(number)", N_("The hyperbolic cosine.") },
  { "COT", "COT(number)", N_("The cotangent.") },
  { "COTH", "COTH(number)", N_("The hyperbolic cotangent.") },
  { "COUNT", "COUNT(value1, value2, ...)", N_("How many of the arguments are numbers.") },
  { "COUNTA", "COUNTA(value1, value2, ...)", N_("How many of the arguments are not blank.") },
  { "COUNTBLANK", "COUNTBLANK(range)", N_("How many cells in a range are blank.") },
  { "COUNTIF", "COUNTIF(range, criteria)", N_("How many cells meet a condition such as \">5\" or \"a*\".") },
  { "COUNTIFS", "COUNTIFS(range1, criteria1, range2, criteria2, ...)", N_("How many cells meet every condition.") },
  { "COVARIANCE.S", "COVARIANCE.S(array1, array2)", N_("The sample covariance.") },
  { "CRITBINOM", "CRITBINOM(trials, probability, alpha)", N_("The smallest value whose cumulative binomial distribution reaches alpha.") },
  { "CRONBACH", "CRONBACH(item1, item2, ...)", N_("Cronbach's alpha: how far the items measure one thing.") },
  { "CSC", "CSC(number)", N_("The cosecant.") },
  { "CSCH", "CSCH(number)", N_("The hyperbolic cosecant.") },
  { "CUMIPMT", "CUMIPMT(rate, nper, pv, start_period, end_period, type)", N_("Interest paid between two periods of a loan.") },
  { "CUMPRINC", "CUMPRINC(rate, nper, pv, start_period, end_period, type)", N_("Principal paid between two periods of a loan.") },
  { "DATE2JULIAN", "DATE2JULIAN(date)", N_("The Julian day number of a date.") },
  { "DATE2UNIX", "DATE2UNIX(date)", N_("A date as Unix time: seconds since the start of 1970.") },
  { "DAVERAGE", "DAVERAGE(database, field, criteria)", N_("The mean of a field in the records that match the criteria.") },
  { "DB", "DB(cost, salvage, life, period, month)", N_("Fixed-declining-balance depreciation.") },
  { "DCOUNT", "DCOUNT(database, field, criteria)", N_("How many records match the criteria and hold a number in the field.") },
  { "DCOUNTA", "DCOUNTA(database, field, criteria)", N_("How many records match the criteria and are not blank in the field.") },
  { "DDB", "DDB(cost, salvage, life, period, factor)", N_("Depreciation by the double-declining balance method.") },
  { "DECIMAL", "DECIMAL(text, radix)", N_("Text in another base as a number.") },
  { "DEGREES", "DEGREES(angle)", N_("Radians to degrees.") },
  { "DGET", "DGET(database, field, criteria)", N_("The field of the one record that matches the criteria.") },
  { "DMAX", "DMAX(database, field, criteria)", N_("The largest value of a field in the matching records.") },
  { "DMIN", "DMIN(database, field, criteria)", N_("The smallest value of a field in the matching records.") },
  { "DOLLARDE", "DOLLARDE(fractional_dollar, fraction)", N_("A price quoted as a fraction, as a decimal.") },
  { "DOLLARFR", "DOLLARFR(decimal_dollar, fraction)", N_("A decimal price as a fraction quote.") },
  { "DPRODUCT", "DPRODUCT(database, field, criteria)", N_("The product of a field in the matching records.") },
  { "DSTDEV", "DSTDEV(database, field, criteria)", N_("The sample standard deviation of a field in the matching records.") },
  { "DSTDEVP", "DSTDEVP(database, field, criteria)", N_("Population standard deviation of matching records.") },
  { "DSUM", "DSUM(database, field, criteria)", N_("The sum of a field in the records that match the criteria.") },
  { "DVAR", "DVAR(database, field, criteria)", N_("The sample variance of a field in the matching records.") },
  { "DVARP", "DVARP(database, field, criteria)", N_("Population variance of matching records.") },
  { "EASTERSUNDAY", "EASTERSUNDAY(year)", N_("Easter Sunday, by the Gregorian computus.") },
  { "EFFECT", "EFFECT(nominal_rate, npery)", N_("The effective annual interest rate.") },
  { "ERROR", "ERROR(text)", N_("The error value that text names.") },
  { "ERROR.TYPE", "ERROR.TYPE(error)", N_("A number for each kind of error value.") },
  { "AGGREGATE", "AGGREGATE(function, options, ref1, ...)", N_("One of nineteen aggregates, leaving out hidden rows or errors as the options say.") },
  { "EVEN", "EVEN(number)", N_("Rounds away from zero to an even integer.") },
  { "EXP", "EXP(number)", N_("e raised to a power.") },
  { "EXPM1", "EXPM1(x)", N_("exp(x) - 1, keeping the digits a small x would lose.") },
  { "EXPPOWDIST", "EXPPOWDIST(x, a, b)", N_("The density of the exponential power distribution.") },
  { "F.DIST", "F.DIST(x, deg_freedom1, deg_freedom2, cumulative)", N_("The F distribution, density or cumulative.") },
  { "F.INV", "F.INV(probability, deg_freedom1, deg_freedom2)", N_("The inverse of the left-tailed F distribution.") },
  { "F.TEST", "F.TEST(array1, array2)", N_("The two-tailed F-test probability.") },
  { "FACT", "FACT(number)", N_("The factorial.") },
  { "FACTDOUBLE", "FACTDOUBLE(number)", N_("The double factorial.") },
  { "FALSE", "FALSE()", N_("The logical value FALSE.") },
  { "FILTER", "FILTER(array, include, if_empty)", N_("The rows (or columns) of an array where a condition holds.") },
  { "FISHER", "FISHER(x)", N_("The Fisher transformation.") },
  { "FISHERINV", "FISHERINV(y)", N_("The inverse of the Fisher transformation.") },
  { "FLOOR", "FLOOR(number, significance)", N_("Rounds down, toward zero, to a multiple of significance.") },
  { "FLOOR.MATH", "FLOOR.MATH(number, significance, mode)", N_("Rounds down to a multiple of the significance.") },
  { "FLOOR.PRECISE", "FLOOR.PRECISE(number, significance)", N_("Rounded down towards minus infinity, whatever the sign.") },
  { "FORMULATEXT", "FORMULATEXT(reference)", N_("What was typed into a cell, formula and all.") },
  { "FTEST", "FTEST(array1, array2)", N_("The two-tailed F-test probability that the variances differ.") },
  { "FV", "FV(rate, nper, pmt, pv, type)", N_("The future value of regular payments.") },
  { "GAMMA", "GAMMA(number)", N_("The gamma function.") },
  { "GAUSS", "GAUSS(z)", N_("The probability between the mean and z standard deviations.") },
  { "GCD", "GCD(number1, number2, ...)", N_("The greatest common divisor.") },
  { "GEOMDIST", "GEOMDIST(k, p, cumulative)", N_("How many failures before the first success.") },
  { "GETENV", "GETENV(name)", N_("A value from the machine's environment.") },
  { "GOODFRIDAY", "GOODFRIDAY(year)", N_("Good Friday, two days before Easter.") },
  { "GROWTH", "GROWTH(known_ys, known_xs, new_xs, const)", N_("Values on the exponential curve fitted to the points.") },
  { "HLOOKUP", "HLOOKUP(value, table, row_index, approximate)", N_("Finds a value in the top row and returns from that column.") },
  { "HYPERLINK", "HYPERLINK(link, friendly_name)", N_("The text to show for a link.") },
  { "HYPGEOM.DIST", "HYPGEOM.DIST(sample_s, number_sample, population_s, number_pop, cumulative)", N_("The hypergeometric distribution.") },
  { "HYPGEOMDIST", "HYPGEOMDIST(successes, draws, population_successes, population)", N_("The hypergeometric distribution.") },
  { "IF", "IF(condition, if_true, if_false)", N_("One value if a condition holds, another if not.") },
  { "IFERROR", "IFERROR(value, if_error)", N_("A value, or something else if it is an error.") },
  { "IFNA", "IFNA(value, if_na)", N_("A value, or something else if it is #N/A.") },
  { "IFS", "IFS(test1, value1, test2, value2, ...)", N_("The first value whose test holds.") },
  { "IMARCCOS", "IMARCCOS(complex)", N_("The inverse cosine of a complex number.") },
  { "IMARCCOSH", "IMARCCOSH(complex)", N_("The inverse hyperbolic cosine of a complex number.") },
  { "IMARCCOT", "IMARCCOT(complex)", N_("The inverse cotangent of a complex number.") },
  { "IMARCCOTH", "IMARCCOTH(complex)", N_("The inverse hyperbolic cotangent of a complex number.") },
  { "IMARCCSC", "IMARCCSC(complex)", N_("The inverse cosecant of a complex number.") },
  { "IMARCCSCH", "IMARCCSCH(complex)", N_("The inverse hyperbolic cosecant of a complex number.") },
  { "IMARCSEC", "IMARCSEC(complex)", N_("The inverse secant of a complex number.") },
  { "IMARCSECH", "IMARCSECH(complex)", N_("The inverse hyperbolic secant of a complex number.") },
  { "IMARCSIN", "IMARCSIN(complex)", N_("The inverse sine of a complex number.") },
  { "IMARCSINH", "IMARCSINH(complex)", N_("The inverse hyperbolic sine of a complex number.") },
  { "IMARCTAN", "IMARCTAN(complex)", N_("The inverse tangent of a complex number.") },
  { "IMARCTANH", "IMARCTANH(complex)", N_("The inverse hyperbolic tangent of a complex number.") },
  { "IMCOSH", "IMCOSH(complex)", N_("The hyperbolic cosine of a complex number.") },
  { "IMCOT", "IMCOT(complex)", N_("The cotangent of a complex number.") },
  { "IMCOTH", "IMCOTH(complex)", N_("The hyperbolic cotangent of a complex number.") },
  { "IMCSC", "IMCSC(complex)", N_("The cosecant of a complex number.") },
  { "IMCSCH", "IMCSCH(complex)", N_("The hyperbolic cosecant of a complex number.") },
  { "IMINV", "IMINV(complex)", N_("One divided by a complex number.") },
  { "IMNEG", "IMNEG(complex)", N_("A complex number with its sign turned round.") },
  { "IMSEC", "IMSEC(complex)", N_("The secant of a complex number.") },
  { "IMSECH", "IMSECH(complex)", N_("The hyperbolic secant of a complex number.") },
  { "IMSINH", "IMSINH(complex)", N_("The hyperbolic sine of a complex number.") },
  { "IMTAN", "IMTAN(complex)", N_("The tangent of a complex number.") },
  { "IMTANH", "IMTANH(complex)", N_("The hyperbolic tangent of a complex number.") },
  { "INDEX", "INDEX(range, row, column)", N_("The cell at a row and column of a range.") },
  { "INDIRECT", "INDIRECT(text)", N_("The reference the text names.") },
  { "INT", "INT(number)", N_("Rounds down to the nearest integer.") },
  { "IPMT", "IPMT(rate, per, nper, pv, fv, type)", N_("The interest part of one payment.") },
  { "IRR", "IRR(values, guess)", N_("The internal rate of return of a series of cash flows.") },
  { "ISBLANK", "ISBLANK(value)", N_("TRUE for an empty cell.") },
  { "ISERR", "ISERR(value)", N_("TRUE for any error but #N/A.") },
  { "ISERROR", "ISERROR(value)", N_("TRUE for any error.") },
  { "ISEVEN", "ISEVEN(number)", N_("TRUE for an even number.") },
  { "ISFORMULA", "ISFORMULA(reference)", N_("Whether the cell holds a formula.") },
  { "ISLOGICAL", "ISLOGICAL(value)", N_("TRUE for TRUE or FALSE.") },
  { "ISNA", "ISNA(value)", N_("TRUE for #N/A.") },
  { "ISNONTEXT", "ISNONTEXT(value)", N_("TRUE for anything that is not text.") },
  { "ISNUMBER", "ISNUMBER(value)", N_("TRUE for a number.") },
  { "ISO.CEILING", "ISO.CEILING(number, significance)", N_("Rounds up to a multiple of the significance.") },
  { "ISODD", "ISODD(number)", N_("TRUE for an odd number.") },
  { "ISOMITTED", "ISOMITTED(argument)", N_("Whether a LAMBDA argument was left out.") },
  { "ISOYEAR", "ISOYEAR(date)", N_("The year the ISO 8601 week of a date belongs to.") },
  { "ISPMT", "ISPMT(rate, period, nper, pv)", N_("Interest paid in a period of an even-principal loan.") },
  { "ISPRIME", "ISPRIME(n)", N_("TRUE if the number is prime.") },
  { "ISREF", "ISREF(value)", N_("TRUE if the value is a reference.") },
  { "ISTEXT", "ISTEXT(value)", N_("TRUE for text.") },
  { "ITHPRIME", "ITHPRIME(i)", N_("The ith prime number.") },
  { "KURTP", "KURTP(number1, number2, ...)", N_("The kurtosis of a whole population.") },
  { "LAMBDA", "LAMBDA(name1, ..., calculation)", N_("A function of its own, called at once or through LET.") },
  { "LANDAU", "LANDAU(x)", N_("The Landau distribution, by its integral.") },
  { "LAPLACE", "LAPLACE(x, a)", N_("The density of Laplace's distribution.") },
  { "LCM", "LCM(number1, number2, ...)", N_("The least common multiple.") },
  { "LET", "LET(name1, value1, ..., calculation)", N_("Names values for use in a calculation.") },
  { "LN", "LN(number)", N_("The natural logarithm.") },
  { "LN1P", "LN1P(x)", N_("log(1 + x), keeping the digits a small x would lose.") },
  { "LOG", "LOG(number, base)", N_("The logarithm to a base, 10 if none is given.") },
  { "LOG10", "LOG10(number)", N_("The logarithm to base 10.") },
  { "LOGEST", "LOGEST(known_ys, known_xs, const, stats)", N_("The exponential curve through the points: m and b of y = b*m^x.") },
  { "LOGINV", "LOGINV(probability, mean, standard_dev)", N_("The inverse lognormal distribution.") },
  { "LOGISTIC", "LOGISTIC(x, a)", N_("The density of the logistic distribution.") },
  { "LOGNORM.DIST", "LOGNORM.DIST(x, mean, standard_dev, cumulative)", N_("The lognormal distribution.") },
  { "LOGNORM.INV", "LOGNORM.INV(probability, mean, standard_dev)", N_("The inverse of the cumulative lognormal distribution.") },
  { "LOOKUP", "LOOKUP(value, lookup_vector, result_vector)", N_("Finds a value in one vector and returns from another.") },
  { "MAKEARRAY", "MAKEARRAY(rows, columns, lambda)", N_("An array from a function of the row and column.") },
  { "MAP", "MAP(array, ..., lambda)", N_("Every element of an array through a function.") },
  { "MATCH", "MATCH(value, range, type)", N_("The position of a value in a range.") },
  { "MAX", "MAX(number1, number2, ...)", N_("The largest number.") },
  { "MAXA", "MAXA(value1, value2, ...)", N_("The largest value, counting text as 0 and TRUE as 1.") },
  { "MAXIFS", "MAXIFS(max_range, range1, criteria1, ...)", N_("The largest value among the cells meeting every condition.") },
  { "MDETERM", "MDETERM(array)", N_("The determinant of a square matrix.") },
  { "MEDIAN", "MEDIAN(number1, number2, ...)", N_("The middle value.") },
  { "MIN", "MIN(number1, number2, ...)", N_("The smallest number.") },
  { "MINA", "MINA(value1, value2, ...)", N_("The smallest value, counting text as 0 and TRUE as 1.") },
  { "MINIFS", "MINIFS(min_range, range1, criteria1, ...)", N_("The smallest value among the cells meeting every condition.") },
  { "MIRR", "MIRR(values, finance_rate, reinvest_rate)", N_("The modified internal rate of return.") },
  { "MOD", "MOD(number, divisor)", N_("The remainder, with the sign of the divisor.") },
  { "MROUND", "MROUND(number, multiple)", N_("Rounds to the nearest multiple.") },
  { "MULTINOMIAL", "MULTINOMIAL(number1, number2, ...)", N_("The multinomial of a set of numbers.") },
  { "N", "N(value)", N_("A value as a number; text becomes 0.") },
  { "NA", "NA()", N_("The error value #N/A.") },
  { "NEGBINOM.DIST", "NEGBINOM.DIST(number_f, number_s, probability_s, cumulative)", N_("The negative binomial distribution.") },
  { "NEGBINOMDIST", "NEGBINOMDIST(failures, successes, probability)", N_("The negative binomial distribution.") },
  { "NETWORKDAYS.INTL", "NETWORKDAYS.INTL(start, end, weekend, holidays)", N_("Working days between two dates, with the weekend named.") },
  { "NOMINAL", "NOMINAL(effect_rate, npery)", N_("The nominal annual interest rate.") },
  { "NORM.S.DIST", "NORM.S.DIST(z, cumulative)", N_("The standard normal distribution.") },
  { "NORMALTEST", "NORMALTEST(array)", N_("D'Agostino and Pearson's test for normality, from the skew and kurtosis.") },
  { "NOT", "NOT(logical)", N_("The opposite of a logical value.") },
  { "NPER", "NPER(rate, pmt, pv, fv, type)", N_("How many payments an investment takes.") },
  { "NPV", "NPV(rate, value1, value2, ...)", N_("The net present value of a series of cash flows.") },
  { "NT_D", "NT_D(n)", N_("How many divisors a number has.") },
  { "NT_MU", "NT_MU(n)", N_("The Moebius mu function of a number.") },
  { "NT_OMEGA", "NT_OMEGA(n)", N_("How many distinct primes go into a number.") },
  { "NT_PHI", "NT_PHI(n)", N_("Euler's totient: how many below n are coprime to it.") },
  { "NT_PI", "NT_PI(n)", N_("How many primes there are up to n.") },
  { "NT_RADICAL", "NT_RADICAL(n)", N_("The product of the distinct primes of a number.") },
  { "NT_SIGMA", "NT_SIGMA(n)", N_("What the divisors of a number add up to.") },
  { "NUMBERVALUE", "NUMBERVALUE(text, decimal_separator, group_separator)", N_("Text as a number, ignoring spaces.") },
  { "ODD", "ODD(number)", N_("Rounds away from zero to an odd integer.") },
  { "OFFSET", "OFFSET(reference, rows, cols, height, width)", N_("A reference moved and resized from another.") },
  { "OR", "OR(logical1, logical2, ...)", N_("TRUE if any argument is TRUE.") },
  { "OWENT", "OWENT(h, a)", N_("Owen's T function.") },
  { "PARETO", "PARETO(x, a, b)", N_("The density of Pareto's distribution.") },
  { "PDURATION", "PDURATION(rate, pv, fv)", N_("Periods for an investment to reach a value.") },
  { "PENTECOSTSUNDAY", "PENTECOSTSUNDAY(year)", N_("Pentecost, forty-nine days after Easter.") },
  { "PERCENTILE.EXC", "PERCENTILE.EXC(array, k)", N_("The k-th percentile, exclusive.") },
  { "PERCENTRANK", "PERCENTRANK(array, x, significance)", N_("The rank of a value as a fraction of the set.") },
  { "PERCENTRANK.EXC", "PERCENTRANK.EXC(array, x, significance)", N_("The rank of a value, with the ends counted out.") },
  { "PERCENTRANK.INC", "PERCENTRANK.INC(array, x, significance)", N_("The rank of a value as a fraction of the set, inclusive.") },
  { "PERMUT", "PERMUT(n, k)", N_("How many ordered ways k things can be chosen from n.") },
  { "PERMUTATIONA", "PERMUTATIONA(number, number_chosen)", N_("Permutations with repetition.") },
  { "PFACTOR", "PFACTOR(n)", N_("The smallest prime that goes into a number.") },
  { "PHI", "PHI(x)", N_("The standard normal density.") },
  { "PI", "PI()", N_("3.14159265358979.") },
  { "PMT", "PMT(rate, nper, pv, fv, type)", N_("The payment on a loan.") },
  { "POWER", "POWER(number, power)", N_("A number raised to a power.") },
  { "PPMT", "PPMT(rate, per, nper, pv, fv, type)", N_("The principal part of one payment.") },
  { "PROB", "PROB(x_range, prob_range, lower, upper)", N_("The probability that a value lies between two limits.") },
  { "PRODUCT", "PRODUCT(number1, number2, ...)", N_("Multiplies its arguments.") },
  { "PV", "PV(rate, nper, pmt, fv, type)", N_("The present value of regular payments.") },
  { "QUARTILE.EXC", "QUARTILE.EXC(array, quart)", N_("A quartile, exclusive.") },
  { "QUOTIENT", "QUOTIENT(numerator, denominator)", N_("The integer part of a division.") },
  { "RADIANS", "RADIANS(angle)", N_("Degrees to radians.") },
  { "RAND", "RAND()", N_("A random number between 0 and 1.") },
  { "RANDARRAY", "RANDARRAY(rows, columns, min, max, whole_number)", N_("An array of random numbers.") },
  { "RANDBETWEEN", "RANDBETWEEN(bottom, top)", N_("A random integer in a range.") },
  { "RANK.AVG", "RANK.AVG(number, ref, order)", N_("The rank of a number, averaged over ties.") },
  { "RATE", "RATE(nper, pmt, pv, fv, type, guess)", N_("The interest rate per period of an annuity.") },
  { "RAYLEIGH", "RAYLEIGH(x, sigma)", N_("The density of Rayleigh's distribution.") },
  { "REDUCE", "REDUCE(initial, array, lambda)", N_("An array folded into one value.") },
  { "ROMAN", "ROMAN(number, form)", N_("A number as a Roman numeral.") },
  { "ROUND", "ROUND(number, digits)", N_("Rounds to a number of digits, halves away from zero.") },
  { "ROUNDDOWN", "ROUNDDOWN(number, digits)", N_("Rounds toward zero.") },
  { "ROUNDUP", "ROUNDUP(number, digits)", N_("Rounds away from zero.") },
  { "ROW", "ROW(reference)", N_("The row number of a reference, or of this cell.") },
  { "RRI", "RRI(nper, pv, fv)", N_("The rate that grows pv to fv over nper periods.") },
  { "SCAN", "SCAN(initial, array, lambda)", N_("The running results of folding an array.") },
  { "SEC", "SEC(number)", N_("The secant.") },
  { "SECH", "SECH(number)", N_("The hyperbolic secant.") },
  { "SEQUENCE", "SEQUENCE(rows, columns, start, step)", N_("An array of numbers in sequence.") },
  { "SERIESSUM", "SERIESSUM(x, n, m, coefficients)", N_("A power series.") },
  { "SHEET", "SHEET(reference)", N_("The number of the sheet, counting from one.") },
  { "SHEETS", "SHEETS(reference)", N_("How many sheets the book has.") },
  { "SIGN", "SIGN(number)", N_("1, 0 or -1 by the sign of a number.") },
  { "SIN", "SIN(number)", N_("The sine of an angle in radians.") },
  { "SINH", "SINH(number)", N_("The hyperbolic sine.") },
  { "SKEW.P", "SKEW.P(number1, number2, ...)", N_("The skewness of a population.") },
  { "SKEWP", "SKEWP(number1, number2, ...)", N_("The skew of a whole population.") },
  { "SLN", "SLN(cost, salvage, life)", N_("Straight-line depreciation for one period.") },
  { "SNORM.DIST.RANGE", "SNORM.DIST.RANGE(x1, x2)", N_("The chance a standard normal falls between the two.") },
  { "SORT", "SORT(array, sort_index, sort_order, by_col)", N_("An array sorted by one of its columns.") },
  { "SORTBY", "SORTBY(array, by_array, sort_order)", N_("An array sorted by another.") },
  { "SQRT", "SQRT(number)", N_("The square root.") },
  { "SQRTPI", "SQRTPI(number)", N_("The square root of a number times pi.") },
  { "SSMEDIAN", "SSMEDIAN(array, interval)", N_("The median of grouped data, interpolated inside its bin.") },
  { "STDEVA", "STDEVA(value1, value2, ...)", N_("Sample standard deviation, counting text as 0 and TRUE as 1.") },
  { "STDEVPA", "STDEVPA(value1, value2, ...)", N_("Population standard deviation, counting text as 0 and TRUE as 1.") },
  { "STEYX", "STEYX(known_ys, known_xs)", N_("The standard error of the predicted y for each x.") },
  { "SUBTOTAL", "SUBTOTAL(function_num, ref1, ...)", N_("An aggregate chosen by number: 1 AVERAGE, 2 COUNT, 3 COUNTA, 4 MAX, 5 MIN, 6 PRODUCT, 7 STDEV, 8 STDEVP, 9 SUM, 10 VAR, 11 VARP.") },
  { "SUM", "SUM(number1, number2, ...)", N_("Adds its arguments; text in a range is skipped.") },
  { "SUMA", "SUMA(value1, value2, ...)", N_("The sum, counting text as 0 and TRUE as 1.") },
  { "SUMIF", "SUMIF(range, criteria, sum_range)", N_("Adds the cells that meet a condition.") },
  { "SUMIFS", "SUMIFS(sum_range, range1, criteria1, ...)", N_("Adds the cells meeting every condition.") },
  { "SUMPRODUCT", "SUMPRODUCT(array1, array2, ...)", N_("Multiplies ranges cell by cell and adds the products.") },
  { "SUMSQ", "SUMSQ(number1, number2, ...)", N_("The sum of the squares.") },
  { "SUMX2MY2", "SUMX2MY2(array_x, array_y)", N_("The sum of the differences of squares.") },
  { "SUMX2PY2", "SUMX2PY2(array_x, array_y)", N_("The sum of the sums of squares.") },
  { "SUMXMY2", "SUMXMY2(array_x, array_y)", N_("The sum of the squares of differences.") },
  { "SWITCH", "SWITCH(value, case1, result1, ..., default)", N_("The result matching a value, or the default.") },
  { "SYD", "SYD(cost, salvage, life, period)", N_("Sum-of-years'-digits depreciation for one period.") },
  { "T", "T(value)", N_("A value if it is text, otherwise empty text.") },
  { "T.DIST", "T.DIST(x, deg_freedom, cumulative)", N_("Student's t distribution, density or cumulative.") },
  { "T.DIST.2T", "T.DIST.2T(x, deg_freedom)", N_("Both tails of Student's t distribution.") },
  { "T.DIST.RT", "T.DIST.RT(x, deg_freedom)", N_("The right tail of Student's t distribution.") },
  { "T.INV", "T.INV(probability, deg_freedom)", N_("The inverse of the left-tailed t distribution.") },
  { "T.TEST", "T.TEST(array1, array2, tails, type)", N_("Student's t-test probability.") },
  { "TAN", "TAN(number)", N_("The tangent of an angle in radians.") },
  { "TANH", "TANH(number)", N_("The hyperbolic tangent.") },
  { "TEXTAFTER", "TEXTAFTER(text, delimiter, [instance], [match_mode], [match_end], [if_not_found])", N_("The part of a text after the delimiter, or the nth of them.") },
  { "TEXTBEFORE", "TEXTBEFORE(text, delimiter, [instance], [match_mode], [match_end], [if_not_found])", N_("The part of a text before the delimiter, or the nth of them.") },
  { "TEXTJOIN", "TEXTJOIN(delimiter, ignore_empty, text1, ...)", N_("Joins texts with a delimiter between them.") },
  { "TREND", "TREND(known_ys, known_xs, new_xs, const)", N_("Values on the line fitted to the points.") },
  { "TRUE", "TRUE()", N_("The logical value TRUE.") },
  { "TRUNC", "TRUNC(number, digits)", N_("Cuts a number off at a number of digits.") },
  { "TTEST", "TTEST(array1, array2, tails, type)", N_("Student's t-test probability: type 1 paired, 2 equal variance, 3 unequal.") },
  { "TYPE", "TYPE(value)", N_("A number for the kind of value: 1, 2, 4, 16 or 64.") },
  { "UNICHAR", "UNICHAR(number)", N_("The character with a Unicode code point.") },
  { "UNICODE", "UNICODE(text)", N_("The code point of the first character.") },
  { "UNIQUE", "UNIQUE(array, by_col, exactly_once)", N_("The distinct rows (or columns) of an array.") },
  { "UNIX2DATE", "UNIX2DATE(t)", N_("Unix time as a date and time.") },
  { "VALUETOTEXT", "VALUETOTEXT(value, format)", N_("A value as text, plainly or with its quotes.") },
  { "VARA", "VARA(value1, value2, ...)", N_("Sample variance, counting text as 0 and TRUE as 1.") },
  { "VARPA", "VARPA(value1, value2, ...)", N_("Population variance, counting text as 0 and TRUE as 1.") },
  { "VDB", "VDB(cost, salvage, life, start, end, factor, no_switch)", N_("Declining-balance depreciation over part of a life.") },
  { "VLOOKUP", "VLOOKUP(value, table, col_index, approximate)", N_("Finds a value in the first column and returns from that row.") },
  { "WORKDAY.INTL", "WORKDAY.INTL(start, days, weekend, holidays)", N_("The date so many working days on, with the weekend named.") },
  { "XIRR", "XIRR(values, dates, guess)", N_("The internal rate of return of dated cash flows.") },
  { "XLOOKUP", "XLOOKUP(lookup, lookup_array, return_array, if_not_found, match_mode)", N_("Finds a value in one vector and returns the matching one from another.") },
  { "XMATCH", "XMATCH(lookup_value, lookup_array, match_mode)", N_("The position of a value in a vector.") },
  { "XNPV", "XNPV(rate, values, dates)", N_("The net present value of dated cash flows.") },
  { "XOR", "XOR(logical1, logical2, ...)", N_("TRUE if an odd number of arguments are TRUE.") },
  { "Z.TEST", "Z.TEST(array, x, sigma)", N_("The one-tailed probability of a z-test.") },
  { "ZTEST", "ZTEST(array, x, sigma)", N_("The one-tailed probability of a z-test.") },
};
...
eval_call_operand (O42EvalContext *ctx, const O42Node *node)
{
...

  {
    ...
    for (int i = 0; i < n_args; i++)
      if (operands[i].is_range && (operands[i].sheet_last != NULL || union_areas (&operands[i]) != NULL))
        any_3d = TRUE;
    if (any_3d)
      {
        ...
        for (int i = 0; i < n_args; i++)
          {
            ...
            GArray *areas = union_areas (&operands[i]);
            int n = 0;
            if (areas != NULL)
              {
                /* A union's areas, each an argument of its own, which is
                 * what SUM((A1:A2,C1:C2)) adds up. */
                for (guint k = 0; k < areas->len; k++)
                  {
                    O42Operand one = g_array_index (areas, O42Operand, k);
    ...
    case O42_NODE_BINARY:
      {
        ...
        if (union_areas (&oa) != NULL || union_areas (&ob) != NULL)
          {
            /* Arithmetic has no shape for a union: #VALUE!, as Excel. */
            operand_clear (&oa);
            operand_clear (&ob);
            op.value = o42_value_error (O42_ERR_VALUE);
            return op;
    ...
    case O42_NODE_UNARY:
      {
        ...
        if (node->as.op.op == O42_OP_IMPLICIT)
          {
            op = implicit_intersection (ctx, &oa);
            operand_clear (&oa);
            return op;
          }
