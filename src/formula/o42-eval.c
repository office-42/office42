...
static O42Value
operand_value (O42EvalContext *ctx, const O42Operand *op)
{
  O42Value v;
...
typedef struct {
  const char  *name;
  int          min_args;
  int          max_args;   /* -1 for "as many as you like" */
  O42FuncImpl  impl;
} O42Function;

#define ARG_NUMBER(index, target)                                        \
  G_STMT_START {                                                         \
    O42Value v_ = operand_value (ctx, &args[index]);                     \
...
static gboolean
collect_numbers (O42EvalContext *ctx, O42Operand *args, int n,
...
{
  /* The table is sorted, so this is a binary search. */
  int lo = 0;
  int hi = (int) G_N_ELEMENTS (FUNCTIONS) - 1;

  while (lo <= hi)
    {
      int mid = (lo + hi) / 2;
      int cmp = g_ascii_strcasecmp (name, FUNCTIONS[mid].name);

      if (cmp == 0) return &FUNCTIONS[mid];
      if (cmp < 0)  hi = mid - 1;
      else          lo = mid + 1;
    }
  ...
  if (all_names == NULL)
    {
      ...
      for (guint i = 0; i < G_N_ELEMENTS (FUNCTIONS); i++)
        g_ptr_array_add (all_names, (gpointer) FUNCTIONS[i].name);
      for (guint i = 0; i < G_N_ELEMENTS (ARRAY_FUNCTIONS); i++)
...
eval_call (O42EvalContext *ctx, const O42Node *node)
{
  ...
  result = fn != NULL ? fn->impl (ctx, operands, n_args)
